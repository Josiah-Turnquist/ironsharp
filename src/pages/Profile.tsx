...
import { Flame, BookOpen, Settings, LogOut, Star, ChevronRight } from "lucide-react";
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: profile } = await supabase
        .from("profiles")
        .select("display_name, streak_count, church_name, survey_church_name, membership_tier")
        .eq("user_id", user.id)
        .maybeSingle();
      ...
      setStats({
        ...
        completed: count ?? 0,
        church: profile?.church_name ?? profile?.survey_church_name ?? "",
      });
    ...
    <AppLayout>
      <div className="mx-auto max-w-lg px-6 py-8">
        ...
        <div className="mb-6 flex flex-col items-center">
          ...
          <h1 className="font-serif text-xl font-bold">{name}</h1>
