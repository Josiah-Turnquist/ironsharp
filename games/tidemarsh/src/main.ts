...
// Game state
// ----------------------------------------------------------------------------
type BuildSel = StructKind | "crop" | null;
...
function tryPlant(c: number, r: number) {
...
function updateCrops(dt: number, water: number) {
  for (let i = 0; i < state.crops.length; i++) {
    ...
    if (crop.growth < 1) crop.growth = Math.min(1, crop.growth + dt / CROP_GROW_SECONDS);
  }
}
  ...
  try {
    ...
    state = {
      ...
      crops: Array.isArray(d.crops) && d.crops.length === COLS * ROWS
        ? d.crops.map((c: any) => (c ? { growth: c.g, dead: !!c.d } : null))
...
function drawHUD(water: number) {
  ...
  ctx.font = "11px Segoe UI, sans-serif";
  ctx.fillText(
    state.buildSel ? "click an adjacent tile · [0] cancel" : "pick 1/2/3, then click a tile",
    bx + 4,
    by + 24,
  );
  if (state.messageT > 0) {
