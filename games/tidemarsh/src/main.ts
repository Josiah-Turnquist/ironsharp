...
interface StructDef {
  ...
  /** deck height relative to the tile's own elevation; clamped to a floor. */
  deckOffset: number;
  deckFloor: number;
}
...
function elevColor(e: number): string {
  ...
  if (e < 0.3) return "#6f5a3e"; // wet mud
  if (e < 0.45) return "#8a724a"; // mudflat
  if (e < 0.6) return "#6f8f55"; // low marsh
  if (e < 0.78) return "#5e9a52"; // marsh grass
...
function drawBuildGhost(water: number) {
  if (!state.buildSel) return;
  const def = STRUCTS[state.buildSel];
  const dirs = [
    [1, 0],
  ...
  for (const [dc, dr] of dirs) {
    ...
    if (state.structs[idx(c, r)] || (c === state.home.c && r === state.home.r)) continue;
...
function drawHUD(water: number) {
  ...
  ctx.fillText(
    `${info.label}${isSpring ? "  ·  SPRING TIDE" : ""}  ${info.rising ? "▲" : "▼"}`,
    gx,
    gy - 2,
  );
  ...
  const opts: { key: string; kind: StructKind }[] = [
  ...
  ctx.font = "11px Segoe UI, sans-serif";
  ctx.fillText(
    state.buildSel ? "click an adjacent tile to build · [0] cancel" : "press 1 or 2 to pick a structure",
    bx + 4,
    by + 24,
  );
...
function frame(now: number) {
  ...
  if (newCycle > prevCycle) state.day++;
  ...
  const time = state.t;
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) drawTile(c, r, water, time);
  }
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) drawStruct(c, r, water);
  }
