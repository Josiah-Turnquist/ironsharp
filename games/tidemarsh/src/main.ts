...
// Structures
// ----------------------------------------------------------------------------
type StructKind = "boardwalk" | "platform";

interface StructDef {
  kind: StructKind;
  label: string;
  cost: { wood: number; reed: number; clam: number };
  deckOffset: number;
  deckFloor: number;
}

const STRUCTS: Record<StructKind, StructDef> = {
  boardwalk: {
    kind: "boardwalk",
    label: "Boardwalk",
...
// ----------------------------------------------------------------------------
// Resources / foraging / food
// ----------------------------------------------------------------------------
type ResKind = "wood" | "reed" | "clam";
const RES_ORDER: ResKind[] = ["wood", "reed", "clam"];
const RES_META: Record<ResKind, { name: string; color: string }> = {
...
// Crop varieties — the farming decision, tied to the weather forecast:
//  greens are quick and cheap (harvest before a storm), roots are slow but
//  far more productive (great in calm stretches, exposed if a surge catches them).
type CropKind = "greens" | "roots";
const CROP_DEFS: Record<CropKind, { name: string; grow: number; yield: number; stalk: string; head: string }> = {
  greens: { name: "Greens", grow: 45, yield: 1, stalk: "#79b94e", head: "#cde06a" },
  roots: { name: "Roots", grow: 92, yield: 3, stalk: "#9a8a4e", head: "#e0b24a" },
};

// Cooking — turn raw produce + clams into meals worth more than the sum of parts.
// Recipes are tried in order; cooking once makes the best meal you can afford.
interface Recipe {
  name: string;
  produce: number;
  clam: number;
...
type Want =
  | { type: "resource"; res: ResKind; amount: number }
...
interface Villager {
  ...
  hat: number;
  want: Want | null;
  cooldown: number; // >0 = recently satisfied, no active request
}

function rollWant(): Want {
  if (Math.random() < 0.4) {
...
function spawnParticles(c: number, r: number, color: string, n: number, spread = 1) {
...
// Game state
// ----------------------------------------------------------------------------
type BuildSel = StructKind | CropKind | null;

interface State {
  elev: number[];
  structs: (StructKind | null)[];
  crops: (Crop | null)[];
  nodes: ForageNode[];
  home: { c: number; r: number };
  player: { c: number; r: number };
  carry: Record<ResKind, number>;
  stored: Record<ResKind, number>;
  produce: number; // harvested crop yield, used for cooking
  food: number;
  t: number;
  day: number;
  buildSel: BuildSel;
...
// ----------------------------------------------------------------------------
function generateTerrain(seed: number): { elev: number[]; home: { c: number; r: number } } {
...
// Tide / lunar helpers
// ----------------------------------------------------------------------------
function cycleIndexAt(t: number): number {
  return Math.floor(t / CYCLE_SECONDS);
}
function isSpringCycle(ci: number): boolean {
...
function tidePhaseInfo(t: number): { rising: boolean; label: string; nextHighIn: number; nextLowIn: number } {
  const frac = cycleFrac(t);
  const rising = frac < 0.5;
  let label: string;
  if (frac < 0.08 || frac > 0.92) label = "LOW TIDE";
  else if (frac > 0.42 && frac < 0.58) label = "HIGH TIDE";
...
function moonPhase(ci: number): number {
  return (ci % LUNAR_CYCLES) / LUNAR_CYCLES; // 0 new, .5 full
}
/** cycles until the next spring tide (0 = this cycle is a spring). */
function cyclesToSpring(ci: number): number {
  let n = 0;
  while (!isSpringCycle(ci + n)) n++;
  return n;
}

function deckHeight(c: number, r: number): number {
...
function isWalkable(c: number, r: number, water: number): boolean {
...
function spawnNodes() {
  ...
  while (state.nodes.length < FORAGE_TARGET && guard < 400) {
    ...
    if (e > 0.5) continue;
    if (c === state.home.c && r === state.home.r) continue;
    if (state.nodes.some((n) => n.c === c && n.r === r)) continue;
    if (state.crops[idx(c, r)]) continue;
    const kind: ResKind = e < 0.32 ? (rng() < 0.5 ? "clam" : "wood") : rng() < 0.5 ? "reed" : "wood";
...
function tryPlant(kind: CropKind, c: number, r: number) {
...
function updateCrops(dt: number, water: number) {
  for (let i = 0; i < state.crops.length; i++) {
    ...
    if (crop.growth < 1) crop.growth = Math.min(1, crop.growth + dt / CROP_DEFS[crop.kind].grow);
  }
}
...
// ----------------------------------------------------------------------------
function bestRecipe(): Recipe | null {
  for (const r of RECIPES) {
    if (state.produce >= r.produce && state.stored.clam >= r.clam) return r;
  }
  return null;
}
function tryCook() {
...
// Building
// ----------------------------------------------------------------------------
function canAfford(def: StructDef): boolean {
  return (
...
function tryBuild(c: number, r: number) {
...
// Villagers
// ----------------------------------------------------------------------------
function wantText(w: Want): string {
...
function tryFulfillVillager() {
  ...
  if (!v || !v.want || v.cooldown > 0) return;
  const w = v.want;
...
// ----------------------------------------------------------------------------
function tryMove(dc: number, dr: number) {
...
function loadGame(): boolean {
  ...
  try {
    ...
    if (d.v !== SAVE_VERSION || typeof d.seed !== "number") return false;
    ...
    state = {
      ...
      crops: Array.isArray(d.crops) && d.crops.length === COLS * ROWS
        ? d.crops.map((c: any) => (c ? { kind: c.k === "roots" ? "roots" : "greens", growth: c.g, dead: !!c.d } : null))
...
function elevColor(e: number): string {
  if (e < 0.3) return "#6f5a3e";
  if (e < 0.45) return "#8a724a";
  if (e < 0.6) return "#6f8f55";
  if (e < 0.78) return "#5e9a52";
  return "#7caa5a";
}
...
function drawTile(c: number, r: number, water: number, time: number) {
  const x = c * TILE;
...
function drawStruct(c: number, r: number, water: number) {
  ...
  if (kind === "boardwalk") {
    ...
    for (let i = 0; i < 4; i++) ctx.fillRect(x + 4 + i * 6, y + 10, 2, TILE - 20);
  } else {
...
function drawCrop(c: number, r: number) {
  ...
  if (ripe) {
    ...
    for (let i = -1; i <= 1; i++) ctx.fillRect(cx + i * 4 - 1.5, baseY - hgt - 2, 4, 4);
  }
}

function drawNode(n: ForageNode, water: number) {
  const x = n.c * TILE;
...
function drawResIcon(kind: ResKind, x: number, y: number) {
...
function drawBuildGhost() {
  ...
  const def = isCrop ? null : STRUCTS[state.buildSel as StructKind];
  const dirs = [
    [1, 0],
    [-1, 0],
    [0, 1],
...
// ---- HUD ----
function drawMoon(x: number, y: number, rad: number, phase: number) {
  ...
  const w = rad * 2 * illum;
  if (waxing) ctx.fillRect(x + rad - w, y - rad, w, rad * 2);
  else ctx.fillRect(x - rad, y - rad, w, rad * 2);
  ctx.restore();
...
function drawHUD(water: number) {
  ...
  const opts: { key: string; sel: BuildSel; label: string; cost: string; afford: boolean }[] = [
  ...
  ctx.font = "11px Segoe UI, sans-serif";
  ctx.fillText(
    state.buildSel ? "click an adjacent tile · [0] cancel" : "pick 1-4 + click · [C] cook at home",
    bx + 6,
    by + 24,
  );
  if (state.messageT > 0) {
...
function frame(now: number) {
  ...
  if (!state.gameOver) {
    ...
    state.t += dt;
    if (cycleIndexAt(state.t) > prevCycle) onNewDay();
    state.messageT -= dt;
  ...
  for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) drawTile(c, r, water, state.t);
  for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) drawStruct(c, r, water);
  for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) drawCrop(c, r);
  for (const n of state.nodes) drawNode(n, water);
  drawHome();
  drawBuildGhost();
  drawVillagers();
  drawPlayer();
...
canvas.addEventListener("mousedown", (e) => {
  ...
  if (py < HUD_TOP || py >= HUD_TOP + GRID_H) return;
