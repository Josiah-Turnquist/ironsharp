...
function isStormCycle(ci) {
    if (ci <= 0)
        return false; // a calm first day to find your feet
...
function updateRain(dt) {
    ...
    if (intensity < 0.05 && rain.length)
        rain = rain.slice(0, Math.max(0, rain.length - 3));
}
function drawRain() {
    if (!rain.length)
        return;
